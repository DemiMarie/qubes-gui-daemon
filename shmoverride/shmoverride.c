...
// The next two functions are purely for error checking by the compiler
// They check that libc has provided prototypes for mmap and mmap64,
// and therefore that the declarations below are correct.
__attribute__((unused))
static void* call_mmap64(void *shmaddr, size_t len, int prot, int flags,
                         int fd, off_t offset) {
    return mmap64(shmaddr, len, prot, flags, fd, offset);
}

__attribute__((unused))
static void* call_mmap(void *shmaddr, size_t len, int prot, int flags,
                       int fd, off_t offset) {
    return mmap(shmaddr, len, prot, flags, fd, offset);
}
